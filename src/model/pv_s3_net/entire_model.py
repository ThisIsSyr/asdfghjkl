...
from src.model.pv_s3_net.losses import *
from itertools import chain
from src.model.pv_s3_net.base_model import BaseModel
from src.model.pv_s3_net.encoder_decoder import *

# 预训练 ResNet 路径：指向项目 weights/pretrained/ 目录
_PRETRAINED_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "..", "weights", "pretrained")
res_net = os.path.join(_PRETRAINED_DIR, "resnet{}.pth")
res_net_2 = os.path.join(_PRETRAINED_DIR, "resnet{}.pth")
...
class EntireModel(BaseModel):
    def __init__(self, num_classes, config, sup_loss=None, cons_w_unsup=None, ignore_index=None,
        ...
        self.unsuper_loss_mse = nn.MSELoss()
    ...
    def forward(self, x_l=None, target_l=None, x_ul=None, target_ul=None, curr_iter=None, epoch=None, id=0,
                warm_up=False, image_level_label=None, semi_p_th=0.6, semi_n_th=0.8):
        if warm_up:
        ...
        loss_unsup_md = loss_unsup * self.unsup_loss_w(epoch=epoch, curr_iter=curr_iter)
        #loss_unsup_mse = self.unsuper_loss_mse(output_ul, target_ul)

        total_loss = loss_unsup_md + loss_sup
        """
